% Analyze how fluctuations in mood relate to mean symptoms
...
% Define datasets, R2 filters, fluctuation metrics, and questionnaires
datasets   = {'dense', 'all', 'monthly'};
r2_threshs = [-8, 0.1];
fluct_params = {'sd_raw','range_raw','rmssd_raw','euclnorm_betas','auc_deviation'}; % add back here if needed
...
% Preallocate results matrices (rows = fluctuation metric x questionnaire, cols = dataset x r2 filter)
n_cond = numel(datasets) * numel(r2_threshs);
n_rows = numel(fluct_params) * numel(q_vars);
rho_mat    = nan(n_rows, n_cond);
p_mat      = nan(n_rows, n_cond);
col_labels = {};

% Loop over datasets and R2 filters, compute fluctuation metrics, join tables, run correlations
col = 0;
for di = 1:numel(datasets)
    ...
    for ri = 1:numel(r2_threshs)
        ...
        % Filter by happiness model R2 and compute fluctuation metrics
        d = happydata_map.(tag);
        d = d(cell2mat(d.r2_evrpe) > r2_threshs(ri), :);
        d.sd_raw           = cellfun(@(s) std(s.rawhappy),                              d.result_evrpe);
        d.range_raw        = cellfun(@(s) max(s.rawhappy) - min(s.rawhappy),            d.result_evrpe);
        %d.range_raw_z      = cellfun(@(s) range(zscore(s.rawhappy)),                    d.result_evrpe);
        d.rmssd_raw        = cellfun(@(s) sqrt(mean(diff(s.rawhappy).^2)),              d.result_evrpe);
        %d.rmssd_raw_z      = cellfun(@(s) sqrt(mean(diff(zscore(s.rawhappy)).^2)),      d.result_evrpe);
        d.euclnorm_betas   = cellfun(@(x) norm(x(1:3)),                                d.b_evrpe);
        d.auc_deviation    = cellfun(@(s) sum(abs(s.rawhappy - mean(s.rawhappy))),      d.result_evrpe);
        ...
        row = 0;
        for pi = 1:numel(fluct_params)
            for qi = 1:numel(q_vars)
                row = row + 1;
                if strcmp(q_src{qi}, 'direct')
                    x = d.phq8_scid; y = d.(fluct_params{pi});
                else
                    tbl = joined.(q_src{qi});
                    x = tbl.(q_vars{qi}); y = tbl.(fluct_params{pi});
                end
...
for pi = 1:numel(fluct_params)
    for qi = 1:numel(q_vars)
        row_labels{end+1} = sprintf('%s x %s', fluct_params{pi}, q_labels{qi});
    end
end
...
% Dividing lines between fluctuation metric groups and datasets
hold on;
for pi = 1:numel(fluct_params)-1
...
title('Spearman correlations: mood fluctuation metrics x questionnaires','FontWeight','bold');
saveas(gcf, 'moodFluctQuestCorrelations_heatmap.png');