function result = fitmodel_itc_hypdisc(indata)
...
[b, ~, exitflag, output, ~, ~, H] = fmincon(@setup_itc_model,result.inx,[],[],[],[],result.lb,result.ub,[],result.options,result);
clear temp;
[loglike, utildiff, logodds, probchoice] = setup_itc_model(b, result);
result.b          = b;
...
function [loglike, utildiff, logodds, probchoice] = setup_itc_model(x, data)

data.mu         = x(1);
data.kappa      = x(2);

[loglike, utildiff, logodds, probchoice] = itc_model(data);

end


function [loglike, utildiff, logodds, probchoice] = itc_model(data)
...
%col 6 chose later is 1, chose sooner is 0).
...
utilsoon     = (data.data(:,2)) ./ (1 + data.kappa.*data.data(:,3)); %utility for sooner option
utillate     = (data.data(:,4)) ./ (1 + data.kappa.*data.data(:,5)); % utility for later option

utildiff      = utillate - utilsoon;            %utility difference between later and sooner options
